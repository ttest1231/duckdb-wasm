...
if [ "${CURRENT_BRANCH}" = "${DEFAULT_BRANCH}" ]; then
    echo "[ RUN ] Install duckdb-wasm-es-app to ${PAGES_DIR}/"
    find ${PAGES_DIR} \
        -mindepth 1 \
        -maxdepth 1 \
        -type d \
    ...
    echo "[ RUN ] Install duckdb-wasm-es typedocs to ${TARGET_DIR}/docs"
    ...
    echo "[ RUN ] Install duckdb-wasm-es-app to ${TARGET_DIR}/"
    rm -rf ${TARGET_DIR}
